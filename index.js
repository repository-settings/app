module.exports = (robot, _, Settings = require('./lib/settings')) => {
  ...
  async function receive (context) {
    ...
    if (defaultBranch && settingsModified) {
      ...
      const result = Buffer.from(repoInfo.data.content, 'base64').toString('ascii')
      ...
      try {
        ...
        if (doc) {
          ...
          await context.github.repos.createStatus({owner: context.repo().owner, repo: context.repo().repo, sha: Commitsha, state: 'success', description: 'Correct settings.yml file'})
        } else {
          context.log('Sorry cannot add')
        }
      } catch (e) {
        context.log(e)
        await context.github.repos.createStatus({owner: context.repo().owner, repo: context.repo().repo, sha: Commitsha, state: 'error', description: 'Validate your settings.yml file, it has syntax errors'})
      }
      const yamltest = jsyml.json2yaml(doc)
      const converted = Buffer.from(yamltest).toString('base64')
