...
class Settings {
  static sync (github, repo) {
    ...
    .then((orgConfig) => {
      if ('noOrgConfig' in orgConfig) {
        ...
        console.info('orgConfig:')
        console.info(orgConfig)
        console.info('orgConfig.data:')
        console.info(orgConfig.data)
        console.info('orgConfig.data.content:')
        console.info(orgConfig.data.content)
  ...
  update () {
    return Promise.all(Object.entries(this.config).map(([section, config]) => {
      ...
      const create_issues = this.config.create_issues
      console.info('INFO', 'create_issues:' + create_issues);
      if(section != "create_issues") {
        const Plugin = Settings.PLUGINS[section]
        console.info('INFO', 'Starting Plugin: '+ section);
        return new Plugin(this.github, this.repo, config, create_issues).sync()
      }
    }))
  }
}
